...
export class CiResultsManager implements vscode.Disposable {
    private client: CiResultsClient | undefined;
    private pollingTimer: NodeJS.Timeout | undefined;
    private disposed = false;
    private knownRunIds = new Set<number>();
    private lastCommitHash: string | undefined;
    private lastRefreshAt: Date | undefined;
    private currentBackendUrl: string | undefined;
    ...
    constructor(
        ...
        private readonly viewProvider: SimpleImpactViewProvider
    ) {
    ...
    async refreshCiResults(manual: boolean = false): Promise<void> {
        if (this.disposed) {
            return;
        }
        ...
        try {
            ...
            const payload: CiResultsPayload = {
                builds,
                commitHash,
                fetchedAt: new Date()
            };
    ...
    private ensureClient(force: boolean = false): CiResultsClient | undefined {
        const backendUrl = this.configurationManager.getBackendUrl().trim();
    ...
    private groupRunsByBuild(runs: CiTestRunEntry[]): CiBuildEntry[] {
        const buildsMap = new Map<number, CiBuildEntry>();
    ...
    private handleNotifications(commitHash: string | undefined, builds: CiBuildEntry[]): void {
        if (this.disposed) {
            return;
        }
        ...
        const newFailures: CiTestRunEntry[] = [];
    ...
    private resetPollingTimer(): void {
        if (this.pollingTimer) {
            clearTimeout(this.pollingTimer);
            this.pollingTimer = undefined;
        }
