...
import { TypeScriptBreakingChangeAnalyzer } from '../analyzers/TypeScriptBreakingChangeAnalyzer';
...
import { BreakingChange as TypeScriptBreakingChange } from '../analyzers/TypeScriptBreakingChangeAnalyzer';
...
export interface ImpactAnalysisResult {
    ...
    breakingChanges?: TypeScriptBreakingChange[]; // TypeScript-aware breaking changes
}
...
export class ProfessionalImpactAnalyzer {
    ...
    private tsBreakingChangeAnalyzer: TypeScriptBreakingChangeAnalyzer;
    ...
    constructor(configManager: ConfigurationManager) {
        ...
        this.tsBreakingChangeAnalyzer = new TypeScriptBreakingChangeAnalyzer();
    ...
    async analyzeFile(filePath: string, document?: vscode.TextDocument, forceAnalysis: boolean = true): Promise<ImpactAnalysisResult> {
        ...
        try {
            ...
            let breakingChanges: TypeScriptBreakingChange[] | undefined;
            ...
            if ((fileExt === '.ts' || fileExt === '.tsx') && before && after) {
                try {
                    ...
                    if (workspaceFolder) {
                        ...
                        breakingChanges = await this.tsBreakingChangeAnalyzer.analyzeBreakingChanges(
                            filePath,
                            before,
                            after,
                        ...
                        if (breakingChanges.length > 0) {
                            const hasHighRiskChange = breakingChanges.some(c => 
                                ['TSAPI-EXP-001', 'TSAPI-EXP-002', 'TSAPI-CLS-001', 'TSX-CMP-002'].includes(c.ruleId)
                            );
            ...
            let finalRiskLevel: 'low' | 'medium' | 'high' = this.calculateRiskLevelFromReport(report);
