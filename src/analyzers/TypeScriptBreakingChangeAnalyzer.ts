...
export type RuleId =
  | "TSAPI-EXP-001" | "TSAPI-EXP-002"
  | "TSAPI-FN-001"  | "TSAPI-FN-002"
  | "TSAPI-CLS-001"
  | "TSAPI-TYP-001" | "TSAPI-TYP-002"
  | "TSX-CMP-001"   | "TSX-CMP-002";

export type ChangeKind =
  | "export_removed"
  | "export_kind_changed"
  | "signature_required_param_added"
  | "signature_param_became_required"
  | "signature_param_type_narrowed"
  | "class_member_removed_or_privatised"
  | "type_required_prop_added_or_made_required"
  | "type_prop_removed_or_narrowed"
  | "component_props_stricter"
  | "component_removed";

export type SymbolKind =
  | "function" | "class" | "type" | "enum" | "value" | "component"
  | "method" | "property";
...
export interface BreakingChange {
  ...
  impactedFileLocations?: Array<{ filePath: string; line: number; column: number }>;
}
...
interface ExportSnap {
  exportName: string;
  exportKind: ExportKind;
  symbolKind: SymbolKind;
  signatures?: SignatureSnap[];
  classMembers?: MemberSnap[];
  typeMembers?: MemberSnap[];
  span: { start: number; end: number };
  typeText?: string;
}

interface ModuleApiSnapshot {
  filePath: string;
  exports: Map<string, ExportSnap>;
}
...
export class TypeScriptBreakingChangeAnalyzer {
  ...
  async analyzeBreakingChanges(
    ...
    try {
      ...
      const beforeCtx = await this.createProgramFromContent(
        filePath,
        beforeContent,
        projectRoot,
        resolvedTsconfig
      );
      const afterCtx = await this.createProgramFromContent(
        filePath,
        afterContent,
        projectRoot,
        resolvedTsconfig
   ...
   */
  private async createProgramFromContent(
    filePath: string,
    content: string,
    projectRoot: string,
    tsconfigPath: string
   ...
   */
  private findTsconfig(startPath: string, filePath: string): string | null {
   ...
   */
  private buildModuleSnapshot(ctx: ProgramContext, filePath: string): ModuleApiSnapshot {
    ...
    const exportsMap = new Map<string, ExportSnap>();
   ...
   */
  private classifySymbolKind(sym: ts.Symbol, decl: ts.Declaration, checker: ts.TypeChecker): SymbolKind {
   ...
   */
  private attachSignaturesIfCallable(
    snap: ExportSnap,
    sym: ts.Symbol,
    decl: ts.Declaration,
    checker: ts.TypeChecker
    ...
    snap.signatures = callSigs.map(sig => {
      const params = sig.getParameters().map(p => {
        ...
        if (pDecl) {
          ...
          try {
            pType = (checker as any).getAnyType?.() || checker.getTypeAtLocation(decl);
          } catch {
    ...
    if (snap.symbolKind === "function") {
      const looksLikeComponent = snap.signatures.some(s =>
        /JSX\.Element|ReactElement|Element/.test(s.returnTypeText)
      );
  ...
  private attachClassMembers(
    ...
    for (const m of classDecl.members) {
      ...
      const access: MemberSnap["access"] =
        (flags & ts.ModifierFlags.Private) ? "private" :
   ...
   * Get node span
   */
  private getNodeSpan(node: ts.Node): { start: number; end: number } {
  ...
  private diffSnapshots(
    beforeSnap: ModuleApiSnapshot,
    afterSnap: ModuleApiSnapshot
  ): BreakingChange[] {
    ...
    for (const [name, a] of afterExports) {
      ...
      if (!b) continue; // Added exports are not breaking
   ...
   */
  private summarizeExport(e: ExportSnap): string {
    if (e.symbolKind === "class") return `class ${e.exportName}`;
    if (e.symbolKind === "type") return `type ${e.exportName}: ${e.typeText ?? ""}`;
    if (e.signatures?.length) return `${e.exportName}${this.formatSig(e.signatures[0])}`;
   ...
   */
  private formatSig(sig: SignatureSnap): string {
  ...
  private async resolveImpacts(
    ...
    for (const change of breakingChanges) {
      try {
        ...
        change.impactedFileLocations = references.map(ref => {
          ...
          if (sourceFile) {
            ...
            return {
              ...
              line: line + 1, // VS Code uses 1-based line numbers
              column: character + 1
            };
          }
          return {
        ...
        change.impactedTests = this.findTestsForSymbol(
          symbol,
          references,
          repoIndex,
          projectRoot
  ...
  private findSymbolReferences(
    ...
    checker: ts.TypeChecker
  ): Array<{ fileName: string; node: ts.Node }> {
    const references: Array<{ fileName: string; node: ts.Node }> = [];
  ...
  private findTestsForSymbol(
    symbol: ts.Symbol,
    references: Array<{ fileName: string; node: ts.Node }>,
    repoIndex: RepoIndex,
    projectRoot: string
  ): string[] {
   ...
   */
  private testFileReferencesSymbol(
    testFile: string,
    symbol: ts.Symbol,
    projectRoot: string
  ): boolean {
   ...
   */
  private resolveImpactsByImports(
    change: BreakingChange,
    sourceFiles: ts.SourceFile[],
    repoIndex: RepoIndex,
    changedFilePath: string
   ...
   */
  private buildRepoIndex(projectRoot: string): RepoIndex {
...
/**
 * Repository index for impact resolution
 */
interface RepoIndex {
  sourceFiles: string[];
  testFiles: string[];
