...
class SimpleImpactViewProvider {
    ...
    async getFileItems(fileElement) {
        ...
        if (tsBreakingChanges.length > 0) {
            ...
            const totalImpactedFiles = new Set();
            const totalImpactedTests = new Set();
            const totalAffectedFunctions = new Set();
            for (const change of tsBreakingChanges) {
                (change.impactedFiles || []).forEach((f) => totalImpactedFiles.add(f));
                (change.impactedTests || []).forEach((t) => totalImpactedTests.add(t));
            ...
            const breakingItem = new ImpactViewItem(`${isCritical ? '🚨' : '⚠️'} What Will Break (${tsBreakingChanges.length})`, 'breaking-issues', vscode.TreeItemCollapsibleState.Expanded // Expanded by default to show immediately
            );
            ...
            const breakingItem = new ImpactViewItem(`🚨 What Will Break (${legacyBreakingIssues.length})`, 'breaking-issues', vscode.TreeItemCollapsibleState.Expanded);
        ...
        if (detailElement.type === 'breaking-issues') {
            ...
            if (breakingChanges.length > 0) {
                ...
                for (let i = 0; i < breakingChanges.length; i++) {
                    ...
                    const changeItem = new ImpactViewItem(this.formatBreakingChangeNodeLabel(change), 'breaking-change', vscode.TreeItemCollapsibleState.Collapsed);
            ...
            // Evidence section
            const evidence = new ImpactViewItem('Evidence', 'breaking-change-evidence', vscode.TreeItemCollapsibleState.None);
            ...
            if (hasImpacts) {
                const impact = new ImpactViewItem('Impact', 'breaking-change-impact', vscode.TreeItemCollapsibleState.Collapsed);
            ...
            const fixes = new ImpactViewItem('Recommended Fixes', 'breaking-change-actions', vscode.TreeItemCollapsibleState.Collapsed);
            ...
            if (breakingChange.span && breakingChange.span.start >= 0) {
                const location = new ImpactViewItem('Location', 'breaking-change-location', vscode.TreeItemCollapsibleState.None);
            ...
            if (impactedFiles && impactedFiles.length > 0) {
                const filesItem = new ImpactViewItem(`📦 Downstream Impact (${impactedFiles.length})`, 'breaking-change-impact-files', vscode.TreeItemCollapsibleState.Collapsed);
            ...
            if (impactedTests && impactedTests.length > 0) {
                const testsItem = new ImpactViewItem(`🧪 Test Impact (${impactedTests.length})`, 'breaking-change-impact-tests', vscode.TreeItemCollapsibleState.Collapsed);
            ...
            if (breakingChange.symbolKind === 'function' || breakingChange.symbolKind === 'method') {
                const usageItem = new ImpactViewItem(`🧩 Function Impact: ${breakingChange.symbolName}() callers may break`, 'breaking-change-impact-usage', vscode.TreeItemCollapsibleState.None);
            ...
            for (const impactedFile of impactedFiles) {
                const location = locations.find((loc) => loc.filePath === impactedFile);
                const fileItem = new ImpactViewItem(path.basename(impactedFile), 'breaking-change-impact-files', vscode.TreeItemCollapsibleState.None);
                ...
                if (location && location.line > 0) {
                    fileItem.command = {
                        ...
                        arguments: [
                            ...
                            {
                                selection: new vscode.Range(location.line - 1, location.column - 1, location.line - 1, location.column - 1)
                            }
                        ]
                    };
                }
            ...
            for (const testFile of impactedTests) {
                const testItem = new ImpactViewItem(path.basename(testFile), 'breaking-change-impact-tests', vscode.TreeItemCollapsibleState.None);
            ...
            for (const action of actions) {
                const actionItem = new ImpactViewItem(`• ${action}`, 'breaking-change-actions', vscode.TreeItemCollapsibleState.None);
            ...
            const breakingChange = context.breakingChange;
            const evidenceItem = new ImpactViewItem(this.formatBreakingChangeDescription(breakingChange), 'breaking-change-evidence', vscode.TreeItemCollapsibleState.None);
            ...
            const changeItem = new ImpactViewItem('Breaking Change', 'ts-breaking-detail', vscode.TreeItemCollapsibleState.None);
            ...
            items.push(changeItem);
            // Impact section
            const impactItem = new ImpactViewItem('Impact', 'ts-breaking-detail', vscode.TreeItemCollapsibleState.Collapsed);
            ...
            const actionItem = new ImpactViewItem('Recommended Action', 'ts-breaking-detail', vscode.TreeItemCollapsibleState.Collapsed);
            ...
            // Location (if available)
            if (breakingChange.span && breakingChange.span.start > 0) {
                const locationItem = new ImpactViewItem('Location', 'ts-breaking-detail', vscode.TreeItemCollapsibleState.None);
            ...
            if (section === 'impact') {
                ...
                if (breakingChange.impactedFiles && breakingChange.impactedFiles.length > 0) {
                    const filesItem = new ImpactViewItem(`📦 Downstream Impact (${breakingChange.impactedFiles.length})`, 'ts-breaking-detail', vscode.TreeItemCollapsibleState.Collapsed);
                    filesItem.iconPath = new vscode.ThemeIcon('file');
                ...
                if (breakingChange.impactedTests && breakingChange.impactedTests.length > 0) {
                    const testsItem = new ImpactViewItem(`🧪 Test Impact (${breakingChange.impactedTests.length})`, 'ts-breaking-detail', vscode.TreeItemCollapsibleState.Collapsed);
                    testsItem.iconPath = new vscode.ThemeIcon('beaker');
                ...
                if (breakingChange.symbolKind === 'function' || breakingChange.symbolKind === 'method') {
                    const funcItem = new ImpactViewItem(`🧩 Function Impact: ${breakingChange.symbolName}() callers may break`, 'ts-breaking-detail', vscode.TreeItemCollapsibleState.None);
                ...
                for (let i = 0; i < impactedFiles.length; i++) {
                    ...
                    const location = locations.find((loc) => loc.filePath === impactedFile);
                    const fileItem = new ImpactViewItem(path.basename(impactedFile), 'ts-breaking-detail', vscode.TreeItemCollapsibleState.None);
                    fileItem.description = location
                        ? `Line ${location.line}`
                        : path.relative(path.dirname(filePath), impactedFile);
                    fileItem.iconPath = new vscode.ThemeIcon('file');
                ...
                for (const testFile of breakingChange.impactedTests || []) {
                    const testItem = new ImpactViewItem(path.basename(testFile), 'ts-breaking-detail', vscode.TreeItemCollapsibleState.None);
                    testItem.description = path.relative(path.dirname(filePath), testFile);
                    testItem.iconPath = new vscode.ThemeIcon('beaker');
                    testItem.command = {
                        command: 'vscode.open',
                ...
                const actions = this.getRecommendedActions(breakingChange);
                for (const action of actions) {
                    const actionItem = new ImpactViewItem(`• ${action}`, 'ts-breaking-detail', vscode.TreeItemCollapsibleState.None);
                    actionItem.iconPath = new vscode.ThemeIcon('lightbulb');
                    items.push(actionItem);
                }
            }
     ...
     */
    calculateRiskFromBreakingChanges(breakingChanges) {
        // Critical risk rules
     ...
     */
    formatBreakingChangeNodeDescription(change) {
     ...
     */
    formatBreakingChangeDescription(change) {
     ...
     */
    formatBreakingChangeWithBeforeAfter(change) {
     ...
     */
    buildBreakingChangesTooltip(breakingChanges, result, riskLevel, impactedFiles, impactedTests, affectedFunctions) {
        const lines = [];
        ...
        if (affectedFunctions > 0)
            summaryParts.push(`${affectedFunctions} function${affectedFunctions !== 1 ? 's' : ''}`);
        if (impactedTests > 0)
            summaryParts.push(`${impactedTests} test${impactedTests !== 1 ? 's' : ''}`);
        if (impactedFiles > 0)
            summaryParts.push(`${impactedFiles} downstream file${impactedFiles !== 1 ? 's' : ''}`);
     ...
     */
    buildBreakingChangeTooltip(change, filePath) {
        const lines = [];
        lines.push(this.formatBreakingChangeDescription(change));
        if (change.before && change.after) {
            lines.push('');
     ...
     */
    buildBreakingChangeDetailTooltip(change) {
        const lines = [];
     ...
     */
    getLineNumberFromSpan(span, filePath) {
        try {
