...
export interface ConfidenceResult {
    total: number; // 0-100
    statusIcon: '✅' | '⚠️' | '❌';
    status: 'high' | 'acceptable' | 'warning' | 'critical';
...
export interface CodeChange {
    ...
    isFromGit: boolean;
    baselineHash?: string;
    hasActualChanges?: boolean; // true if there are real changes (Git diff or baseline diff), false if forced full-file analysis
}

export class ConfidenceEngine {
    private configManager: ConfigurationManager;
    private gitAnalyzer: GitAnalyzer;
    private baselineCache: Map<string, { content: string; hash: string; timestamp: number }> = new Map();
     ...
     */
    async analyzeChange(change: CodeChange): Promise<ConfidenceResult> {
     ...
     */
    async detectChanges(filePath: string, currentContent?: string, forceAnalysis: boolean = false): Promise<CodeChange | null> {
        try {
     ...
     */
    private async metricCodeCorrectness(change: CodeChange): Promise<MetricResult> {
        ...
        const relevantDiagnostics = diagnostics.filter(d => 
            this.isDiagnosticInChangedLines(d, changedLines)
        );
        ...
        const codeCorrectnessScore = Math.round(
            syntaxResult.score * 0.25 +
        ...
        const allIssues: Array<{ message: string; line: number }> = [];
        ...
        const subMetrics: Record<string, any> = {
            syntax: { 
     ...
     */
    private findLineNumber(pattern: RegExp, code: string, allLines: string[], startLineOffset: number = 0): number[] {
        const lines: number[] = [];
     ...
     */
    private getDiagnosticLineNumber(diagnostic: vscode.Diagnostic, change: CodeChange): number {
     ...
     * Sub-metric 1: Syntax & Parse Validity (25%)
     */
    private subMetricSyntaxValidity(
        diagnostics: vscode.Diagnostic[], 
        changedCode: string, 
        change: CodeChange,
        allLines: string[]
    ): { score: number; issues: Array<{ message: string; line: number }>; lineNumbers: number[] } {
        let score = 100;
        ...
        const conflictMatches: Array<{ line: number; marker: string }> = [];
        ...
        let openBrackets = 0;
        let closeBrackets = 0;
        const bracketIssues: Array<{ line: number; message: string }> = [];
    ...
    private subMetricTypeSafety(
        ...
        const implicitAnyLines: Array<{ line: number; message: string }> = [];
        const changedLineSet = new Set([...change.addedLines, ...change.modifiedLines]);
        ...
        if (unsafeAccessPattern.test(changedCode) && !hasGuards) {
            const unsafeLines = this.findLineNumber(unsafeAccessPattern, changedCode, allLines,
                Math.min(...change.addedLines, ...change.modifiedLines) - 1);
     ...
     * Sub-metric 3: Critical Static Bugs (25%)
     */
    private subMetricCriticalStaticBugs(
        diagnostics: vscode.Diagnostic[], 
        changedCode: string, 
        change: CodeChange,
        allLines: string[]
        ...
        if (missingReturnPattern.test(changedCode)) {
            ...
            if (funcBody && !/return\s+/.test(funcBody[0])) {
                score -= 20;
                const funcLines = this.findLineNumber(/function\s+\w+\s*\(/, changedCode, allLines,
                    Math.min(...change.addedLines, ...change.modifiedLines) - 1);
        ...
        const unsafeLines: Array<{ line: number; name: string }> = [];
        const changedLineSet = new Set([...change.addedLines, ...change.modifiedLines]);
    ...
    private subMetricComplexity(
        ...
        if (complexityCount > 5) {
            ...
            const firstComplexityLine = this.findLineNumber(complexityKeywords, changedCode, allLines,
                Math.min(...change.addedLines, ...change.modifiedLines) - 1);
        ...
        const lineCounts = new Map<string, Array<{ line: number; content: string }>>();
     ...
     * Sub-metric 5: Standards & Style Violations (10%)
     */
    private subMetricStyleViolations(
        diagnostics: vscode.Diagnostic[], 
        changedCode: string,
        change: CodeChange,
        allLines: string[]
        ...
        if (hasCamel && hasSnake) {
            score -= 5;
            const camelLines = this.findLineNumber(camelCase, changedCode, allLines,
                Math.min(...change.addedLines, ...change.modifiedLines) - 1);
            const snakeLines = this.findLineNumber(snakeCase, changedCode, allLines,
                Math.min(...change.addedLines, ...change.modifiedLines) - 1);
        ...
        if (magicNumberPattern.test(changedCode) && !hasConstants) {
            const magicLines = this.findLineNumber(magicNumberPattern, changedCode, allLines,
                Math.min(...change.addedLines, ...change.modifiedLines) - 1);
    ...
    private subMetricSafetyGuards(
        ...
        const riskyOperations: Array<{ pattern: RegExp; name: string }> = [
        ...
        const riskyOpsWithoutGuards: Array<{ line: number; name: string }> = [];
        
        for (const { pattern, name } of riskyOperations) {
            const foundLines = this.findLineNumber(pattern, changedCode, allLines,
                Math.min(...change.addedLines, ...change.modifiedLines) - 1);
        ...
        if (!hasErrorHandling && /async|Promise|await/.test(changedCode)) {
            const asyncLines = this.findLineNumber(/async|await|Promise/, changedCode, allLines,
                Math.min(...change.addedLines, ...change.modifiedLines) - 1);
     ...
     */
    private async metricSecurity(change: CodeChange): Promise<MetricResult> {
        let score = 100;
        const issues: string[] = [];
        ...
        // Insecure API usage
        const insecurePatterns: Array<{pattern: RegExp; severity: 'high' | 'medium' | 'low'; name: string}> = [
     ...
     */
    private async metricTestValidation(change: CodeChange): Promise<MetricResult> {
        let score = 100;
        const issues: string[] = [];
        const subMetrics: Record<string, any> = {
            hasTestFile: false,
     ...
     */
    private async metricContractsArchitecture(change: CodeChange): Promise<MetricResult> {
        let score = 100;
        const issues: string[] = [];
        const subMetrics: Record<string, any> = {
            breakingChanges: [],
        ...
        const changedSignatures: Array<{ name: string; fullSignature: string }> = [];
        ...
        if (changedSignatures.length > 0 && change.originalContent) {
            for (const changedSig of changedSignatures) {
                ...
                const originalPattern = new RegExp(
                    `(?:export\\s+(?:default\\s+)?(?:function|class|const|let|var)\\s+${changedSig.name}\\s*\\([^)]*\\)|export\\s+(?:default\\s+)?class\\s+${changedSig.name}|module\\.exports\\s*=\\s*(?:function|class)\\s+${changedSig.name}\\s*\\([^)]*\\)|exports\\.${changedSig.name}\\s*=\\s*(?:function|class)\\s*\\([^)]*\\)|public\\s+(?:static\\s+)?(?:class|interface|enum)\\s+${changedSig.name}|def\\s+${changedSig.name}\\s*\\([^)]*\\))`,
                    'm'
                );
                ...
                if (!originalMatch) {
                    ...
                    const fullFilePattern = new RegExp(
                        `(?:export\\s+(?:default\\s+)?(?:function|class|const|let|var)\\s+${changedSig.name}\\s*\\([^)]*\\)|export\\s+(?:default\\s+)?class\\s+${changedSig.name})`,
                        'm'
                    );
     ...
     */
    private async metricChangeRisk(change: CodeChange): Promise<MetricResult> {
        let score = 100;
        const issues: string[] = [];
        const subMetrics: Record<string, any> = {
            changeSize: 0,
     ...
     */
    private async metricCodeHygiene(change: CodeChange): Promise<MetricResult> {
        let score = 100;
        const issues: string[] = [];
        const subMetrics: Record<string, any> = {
            formattingIssues: [],
        ...
        for (const line of changedLines) {
            ...
            if (/[\t ]+$/.test(line)) hasTrailingWhitespace = true;
        }
    ...
    private classifyStatus(score: number): 'high' | 'acceptable' | 'warning' | 'critical' {
    ...
    private getStatusIcon(status: string): '✅' | '⚠️' | '❌' {
        if (status === 'high' || status === 'acceptable') return '✅';
    ...
    private parseGitDiff(diff: string): { added: number[]; removed: number[]; modified: number[] } {
    ...
    private computeLineDiff(baseline: string, current: string): { added: number[]; removed: number[]; modified: number[] } {
    ...
    private extractChangedCode(change: CodeChange): string {
    ...
    private isDiagnosticInChangedLines(diagnostic: vscode.Diagnostic, changedLines: number[]): boolean {
    ...
    private async findNearbyTestFiles(filePath: string): Promise<string[]> {
    ...
    private getBaseline(filePath: string): { content: string; hash: string } | null {
        return this.baselineCache.get(filePath) || null;
    }

    private saveBaseline(filePath: string, content: string): void {
        ...
        try {
            ...
            if (fs.existsSync(baselineFile)) {
                ...
                for (const [filePath, baseline] of Object.entries(baselines)) {
                    this.baselineCache.set(filePath, baseline as any);
                }
            }
        } catch (error) {
