...
export interface BreakingChange {
    type: 'signature' | 'return-type' | 'parameter' | 'visibility';
    description: string;
...
export interface TypeChange {
    type: 'interface' | 'type-alias' | 'enum' | 'namespace';
    name: string;
...
export class ProfessionalImpactAnalyzer {
    ...
    async analyzeFileImpact(filePath: string): Promise<ProfessionalAnalysisResult> {
        try {
            ...
            let changedClasses: ChangedClass[];
    ...
    private async analyzeChangedFunctions(sourceFile: SourceFile, changedLines: number[]): Promise<ChangedFunction[]> {
        ...
        for (const func of functions) {
            ...
            if (isFunctionChanged) {
                ...
                const changedFunction: ChangedFunction = {
    ...
    private async analyzeChangedClasses(sourceFile: SourceFile, changedLines: number[]): Promise<ChangedClass[]> {
        ...
        for (const cls of classes) {
            ...
            if (this.isInChangedLines(line, changedLines)) {
                ...
                const changedClass: ChangedClass = {
    ...
    private async analyzeAllFunctions(sourceFile: SourceFile): Promise<ChangedFunction[]> {
        const allFunctions: ChangedFunction[] = [];
    ...
    private async analyzeAllClasses(sourceFile: SourceFile): Promise<ChangedClass[]> {
        const allClasses: ChangedClass[] = [];
        const classes = sourceFile.getClasses();
    ...
    private async findAffectedTestsTypeAware(changedFunctions: ChangedFunction[], changedClasses: ChangedClass[], sourceFilePath: string): Promise<string[]> {
    ...
    private async findDownstreamComponentsTypeAware(changedFunctions: ChangedFunction[], changedClasses: ChangedClass[], sourceFilePath: string): Promise<string[]> {
        ...
        try {
            ...
            const indirectDependencies = await this.findIndirectDependencies(
                downstreamComponents, 
                changedFunctions, 
                changedClasses, 
                sourceFilePath
     ...
     */
    private async findIndirectDependencies(
                    ...
                    try {
                        ...
                        if (!importsDirectDep) {
                            const importLines = content.split('\n').filter((line: string) => 
                                line.includes('import') && (line.includes(directDepBaseName) || line.includes(directDepRelativePathNoExt))
                            );
    ...
    private async detectBreakingChanges(changedFunctions: ChangedFunction[], changedClasses: ChangedClass[]): Promise<BreakingChange[]> {
    ...
    private async analyzeTypeChanges(sourceFile: SourceFile, changedLines: number[]): Promise<TypeChange[]> {
    ...
    private isInChangedLines(line: number, changedLines: number[]): boolean {
        if (changedLines.length === 0) {
    ...
    private async findReferences(symbolName: string): Promise<string[]> {
        // This would use TypeScript's findReferences API
        // For now, return empty array
        return [];
    }

    private async findTestFilesInWorkspace(): Promise<string[]> {
        ...
        try {
            ...
            for (const pattern of testPatterns) {
                ...
                testFiles.push(...matches.map((uri: any) => uri.fsPath));
            }
        } catch (error) {
    ...
    private async findSourceFilesInWorkspace(): Promise<string[]> {
        ...
        try {
            ...
            for (const pattern of sourcePatterns) {
                ...
                sourceFiles.push(...matches.map((uri: any) => uri.fsPath));
            }
        } catch (error) {
    ...
    private isLanguageCompatible(sourceLang: string, targetLang: string): boolean {
     ...
     */
    private removeCommentsAndStrings(content: string, language: string): string {
        let cleaned = content;
        ...
        if (language === 'javascript' || language === 'typescript') {
            ...
            const importPathPlaceholders: string[] = [];
    ...
    private isFunctionUsedInFile(content: string, functionName: string, sourceLang: string, targetLang: string): boolean {
    ...
    private isClassUsedInFile(content: string, className: string, sourceLang: string, targetLang: string): boolean {
    ...
    private async findTestFilesThatImport(): Promise<string[]> {
        // This would find test files that import the current module
        return [];
    }

    private calculateConfidence(changedFunctions: ChangedFunction[], changedClasses: ChangedClass[], affectedTests: string[]): number {
    ...
    private calculateCoverageImpact(changedFunctions: ChangedFunction[], changedClasses: ChangedClass[]): number {
        const totalChanges = changedFunctions.length + changedClasses.length;
        return Math.min(1.0, totalChanges * 0.1);
    }

    private calculateRiskLevel(breakingChanges: BreakingChange[], downstreamComponents: string[]): 'low' | 'medium' | 'high' {
        if (breakingChanges.length > 0) return 'high';
        if (downstreamComponents.length > 5) return 'medium';
        return 'low';
    }
     ...
     */
    private async analyzePythonFile(filePath: string): Promise<ProfessionalAnalysisResult> {
        try {
            ...
            const allClasses: Array<{ name: string; startLine: number; indent: number }> = [];
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (funcMatch || methodMatch) {
                    ...
                    const isInChangedLines = changedLines.some((changedLine: number) => 
                        changedLine >= lineNum && changedLine <= funcEndLine
                    );
            ...
            for (const cls of allClasses) {
                ...
                const isClassChanged = changedLines.some((changedLine: number) => 
                    changedLine === cls.startLine || 
    ...
    private getEmptyResult(filePath: string): ProfessionalAnalysisResult {
        return {
            filePath,
            changedFunctions: [],
            changedClasses: [],
